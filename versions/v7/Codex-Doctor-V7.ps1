...
function Move-CodexData([string]$TargetRoot){
    ...
    if(Test-Path $CodexDir){$i=Get-Item $CodexDir -Force;if($i.Attributes -band [IO.FileAttributes]::ReparsePoint){throw '.codex is already a reparse point.'}}
    ...
    $backup=$null
    if(Test-Path $CodexDir){& robocopy $CodexDir $target /E /COPY:DAT /DCOPY:DAT /R:2 /W:1 /XJ /NFL /NDL /NP|Out-Null;if($LASTEXITCODE -ge 8){throw 'Robocopy failed: '+$LASTEXITCODE};$backup=$CodexDir+'.pre-migrate-'+(Get-Date -Format 'yyyyMMdd-HHmmss');Move-Item $CodexDir $backup}
...
function Export-V7Report($Result){$p=Join-Path $ReportDir ('CodexDoctorV7-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'.json');$Result|ConvertTo-Json -Depth 8|Set-Content $p -Encoding UTF8;return $p}
...
$form=New-Object Windows.Forms.Form;$form.Text='Codex Doctor V7 — Unified GUI';$form.Size=New-Object Drawing.Size(1000,760);$form.MinimumSize=New-Object Drawing.Size(920,700);$form.StartPosition='CenterScreen'
...
function Update-V7Ui($r){$script:LastResult=$r;switch($r.Health){'Healthy'{$script:HealthLabel.ForeColor=[Drawing.Color]::ForestGreen;$script:HealthLabel.Text='● 健康'}'Warning'{$script:HealthLabel.ForeColor=[Drawing.Color]::DarkOrange;$script:HealthLabel.Text='● 需要检查'}default{$script:HealthLabel.ForeColor=[Drawing.Color]::Firebrick;$script:HealthLabel.Text='● 连接故障'}};$d=$r.Diagnosis;$script:DetailLabel.Text="原因：$($d.Class)    DNS=$($d.Dns.Ok)    TLS=$($d.Tls.Ok)    Proxy=$($d.Proxy.Ok)    TUN=$($d.Tun.TunDetected)`nGitConflict=$($d.Git.Conflict)    npmConflict=$($d.Npm.Conflict)    .env=$($r.Env.Exists)    Proxy=$($r.ProxyUrl)";if($r.ProxyUrl -and -not $proxyBox.Text){$proxyBox.Text=$r.ProxyUrl};Write-V7Log ('Diagnosis: '+$d.Class+' | health='+$r.Health+' | proxy='+$r.ProxyUrl)}
...
$btnRepair.Add_Click({try{$r=Invoke-V7Diagnosis -RequestedProxy $proxyBox.Text.Trim();Update-V7Ui $r;if(@($r.Plan.Actions).Count -eq 0){[Windows.Forms.MessageBox]::Show(($r.Plan.Advisory -join "`n"),'当前无自动修复动作');return};$m="诊断：$($r.Diagnosis.Class)`n建议动作：$($r.Plan.Actions -join ', ')`n是否执行？";if([Windows.Forms.MessageBox]::Show($m,'确认修复',[Windows.Forms.MessageBoxButtons]::YesNo) -ne [Windows.Forms.DialogResult]::Yes){return};foreach($a in $r.Plan.Actions){switch($a){'WRITE_CODEX_ENV'{if(-not $r.ProxyUrl){throw '没有已验证代理。'};Set-CodexProxyEnvFile -EnvFile $EnvFile -ProxyUrl $r.ProxyUrl|Out-Null;if($writeUserEnv.Checked){Set-CodexUserProxyEnvironment -ProxyUrl $r.ProxyUrl -Confirm:$false};Write-V7Log 'Updated .codex/.env.'}'CLEAR_GIT_PROXY'{Clear-GitProxyConfig -Confirm:$false;Write-V7Log 'Cleared Git global proxy.'}'CLEAR_NPM_PROXY'{Clear-NpmProxyConfig -Confirm:$false;Write-V7Log 'Cleared npm proxy.'}}};Update-V7Ui (Invoke-V7Diagnosis -RequestedProxy $r.ProxyUrl)}catch{Write-V7Log ('ERROR '+$_.Exception.Message);[Windows.Forms.MessageBox]::Show($_.Exception.Message,'修复失败')}})
